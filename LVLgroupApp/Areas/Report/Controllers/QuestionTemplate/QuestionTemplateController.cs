...
namespace LVLgroupApp.Areas.Report.Controllers.QuestionTemplate
{
    ...
    public class QuestionTemplateController : BaseController<QuestionTemplateController>
    {
                ...
                else
                {
                    ...
                    if (id > 0)
                    {
                        ...
                        if (questionTemplate.UsedInReports > 0)
                        {
                            ...
                            // desativar QuestionTemplate antiga
                            var deactivateQuestionTemplateCommand = new UpdateQuestionTemplateCommand();
                            deactivateQuestionTemplateCommand.Id = id;
                            deactivateQuestionTemplateCommand.IsActive = false;
                            var resultDQT = await _mediator.Send(deactivateQuestionTemplateCommand);
                            if (!resultDQT.Succeeded)
                            {
                                _logger.LogError(_sessionId + " | " + _sessionName + " | QuestionTemplate Contoller - OnPostCreateOrEdit - Exception: " + resultDQT.Message);
                                _notify.Error(resultDQT.Message);
                                // return _ViewAll
                                var html = await _viewRenderer.RenderViewToStringAsync("_ViewAll", new QuestionTemplateViewModel());
                                return new JsonResult(new { isValid = true, html = html });
                            }
        ...
        public static async Task<SelectList> GetSelectListAllQuestionTemplatesAsync(string lang, int selectedId, IMapper mapper, IMediator mediator)
        {
            var questionTemplateLocalizedResponse = await mediator.Send(new GetAllQuestionTemplateLocalizedCachedQuery());
            var questionTemplateLocalizedViewModel = mapper.Map<List<QuestionTemplateLocalizedViewModel>>(questionTemplateLocalizedResponse.Data).AsQueryable();
            questionTemplateLocalizedViewModel = questionTemplateLocalizedViewModel.Where(qtl => qtl.Language == lang);
            return new SelectList(questionTemplateLocalizedViewModel, nameof(QuestionTemplateLocalizedViewModel.Id), nameof(QuestionTemplateLocalizedViewModel.QuestionText), selectedId, null);
        }
        ...
        public static async Task<List<QuestionTemplateViewModel>> GetAllQuestionTemplatesAsync(string lang, IMapper mapper, IMediator mediator, ILogger logger, string sessionId, string sessionName, IRequestCultureFeature cultureFeature)
        {
            try
            {
                // Culture contains the information of the requested culture
                ...
                // lista de Question Templates
                var responseAllQuestionTemplates = await mediator.Send(new GetAllQuestionTemplateCachedQuery());
                if (!responseAllQuestionTemplates.Succeeded) return null;
                var allQuestionTemplatesData = mapper.Map<List<QuestionTemplateViewModel>>(responseAllQuestionTemplates.Data).AsQueryable();

                // lista de Question Templates (localized)
                var responseAllQuestionTemplateLocalized = await mediator.Send(new GetAllQuestionTemplateLocalizedCachedQuery());
                if (!responseAllQuestionTemplateLocalized.Succeeded) return null;
                var allQuestionTemplateLocalizedData = mapper.Map<List<QuestionTemplateLocalizedViewModel>>(responseAllQuestionTemplateLocalized.Data).AsQueryable();

                // lista de Question Options
                var responseAllQuestionOptions = await mediator.Send(new GetAllQuestionOptionCachedQuery());
                if (!responseAllQuestionOptions.Succeeded) return null;
                var allQuestionOptionsData = mapper.Map<List<QuestionOptionViewModel>>(responseAllQuestionOptions.Data).AsQueryable();

                // lista de ReportTemplateQuestions para verificar se a
                // QuestionTemplate está a ser usada em algum Report
                var responseAllReportTemplateQuestions = await mediator.Send(new GetAllReportTemplateQuestionCachedQuery());
                if (!responseAllReportTemplateQuestions.Succeeded) return null;
                var allReportTemplateQuestionsData = mapper.Map<List<Core.Entities.Reports.ReportTemplateQuestion>>(responseAllReportTemplateQuestions.Data).AsQueryable();

                // lista de Question Templates
                var allQuestionTemplatesList = new List<QuestionTemplateViewModel>();
                ...
                {
                    // criar novo QuestionTemplateViewModel
                    var questionTemplate = new QuestionTemplateViewModel();
                ...
                return allQuestionTemplatesList;
            }
            catch (Exception ex)
            {
                logger.LogError(sessionId + " | " + sessionName + " | QuestionTemplate Contoller - GetAllQuestionTemplatesAsync - Exception vai sair e retornar Error: " + ex.Message);
                return new List<QuestionTemplateViewModel>();
            }
        }
